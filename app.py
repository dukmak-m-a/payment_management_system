...
def get_receipts():
    try:
        ...
        for row in res.data:
            ...
            receipt["paymentCodeDisplay"] = (row.get("Payments") or {}).get("PaymentCode")
...
def create_payment():
    ...
    try:
        if data.get("Currency"):
            curr = supabase.table("Projects").select("Currency").eq("id", data["ProjectId"]).single().execute()
            if curr.data != data.get("Currency"):
                return jsonify({"success": False, "error": "currency dismatch with project currency"}), 400
